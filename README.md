...
- Отправка AT-команд на модем через SMS с префиксом `at=`, например `at=AT+EGMREXT=0,7`. Ответ команды придёт в выбранное уведомление
- Выполнение CLI-команд Keenetic через SMS с префиксом `cli=`, например `cli=interface UsbQmi0 show`. `CLI_COMMANDS_ENABLED=1` выполняет команду, `CLI_COMMANDS_ENABLED=2` дополнительно выполняет `system configuration save`. Ответ придёт в выбранное уведомление
- Ответы AT- и CLI-команд длиннее 3500 символов обрезаются перед отправкой
- AT- и CLI-команды проходят проверку чёрного и белого списков отправителей. Для безопасного удалённого управления укажите номер(а) в `WHITE_LIST`
